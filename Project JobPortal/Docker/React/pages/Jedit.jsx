...
const JobSeekerEdit = () => {
        ...
        const fetchUserDetails = async () => {
            try {
                ...
                if (response.ok) {
                    ...
                    setAbout(data.about || "");
                    setPhone(data.phone || "");
                    setHigherqualification(data.higher || "");
                    setAge(data.age || "");
                    setGender(data.gender || "");
                    setEducationQualification(data.education?.[0]?.Qualification || "");
                    setEducationCollege(data.education?.[0]?.College || "");
                    setEducationUniversity(data.education?.[0]?.University || "");
                    setEducationGrade(data.education?.[0]?.Grade || "");

                    setJobTitle(data.experience?.[0]?.JobTitle || "");
                    setCompanyName(data.experience?.[0]?.Company || "");
                    setDetails(data.experience?.[0]?.Details || "");
                    setExperienceYears(data.experience?.[0]?.Years || "");
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await fetch(`/api/jobseeker/edit`, {
                method: "PATCH",
                body: formData,
            });

            if (response.ok) {
                navigate("/jprofile"); // Redirect to dashboard after update
            }
        } catch (error) {
